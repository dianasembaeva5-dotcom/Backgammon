...
interface CosmicSpectrumProps {
  color?:
    ...
    | "beige-black"
  blur?: boolean
  /** Overrides the animated title text. Default: "Cosmos Spectrum" */
  title?: string
  /** Extra content rendered below the title inside the hero section */
  children?: ReactNode
}
...
export function CosmicSpectrum({
  ...
  useEffect(() => {
    const loadScript = (src: string) => {
      return new Promise((resolve, reject) => {
        ...
        if (document.querySelector(`script[src="${src}"]`)) { resolve(null); return }
  ...
  const setupAnimations = () => {
    ...
    navItems.forEach((item) => {
      ...
      heroTl.to(item, { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.8, ease: "power2.out" }, 0.4)
    })
    ...
    // Scroll-triggered spectrum animation
    ...
    gsap.set([...wavelengthLabels, mainTitle].filter(Boolean), { opacity: 0, y: 30, filter: "blur(8px)" })
    ...
    tl.to(".svg-container", { opacity: 1, duration: 0.01 }, 0)
      ...
      .to(
        [...wavelengthLabels, mainTitle].filter(Boolean),
        { duration: 0.8, y: 0, opacity: 1, filter: "blur(0px)", stagger: 0.08, ease: "power2.out" },
        0.9,
      )
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen overflow-x-hidden">
      ...
      <section className="h-screen w-full p-8 flex flex-col items-center justify-center relative">
        ...
        {children && (
          <div className="hero-text mt-8 w-full flex flex-col items-center">
            {children}
          </div>
        )}
      </section>
